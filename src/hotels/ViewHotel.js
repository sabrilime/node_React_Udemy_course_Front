...
const ViewHotel = (match, history) => {
    ...
    useEffect(() => {
        if(auth && auth.token) {
            isAlreadyBooked(auth.token, match.match.params.hotelId)
            .then(res => {
    ...
    const loadSellerHotel = async() => {
        let res = await read(match.match.params.hotelId);
        setHotel(res.data);
    ...
    const handleClick = async (e) => {
        ...
        let res = await getSessionId(auth.token, match.match.params.hotelId);
